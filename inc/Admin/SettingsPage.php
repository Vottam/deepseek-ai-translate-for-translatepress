...
class SettingsPage {
    ...
    /**
     * Render API key field (password, never shows real value).
     *
     * @return void
     */
    public function render_api_key_field(): void {
        $has_key = $this->settings->is_openai_configured();
        ?>
        <input type="password"
               id="openai_api_key"
               ...
               value=""
               class="regular-text"
               placeholder="<?php echo $has_key ? esc_attr__( '•••••••• configured (enter new key to change)', 'hollisho-integration-deepseek-for-translatepress' ) : ''; ?>"
               autocomplete="off"
        >
        <?php if ( $has_key ) : ?>
            <p class="description">
                <?php esc_html_e( 'A key is configured. Enter a new key above to replace it, or clear the field and save to remove.', 'hollisho-integration-deepseek-for-translatepress' ); ?>
            </p>
        <?php else : ?>
            <p class="description">
