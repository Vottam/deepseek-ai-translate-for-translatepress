<?php
/**
 * OpenRouter translation engine for TranslatePress.
 *
...
use hollisho\translatepress\translate\deepseek\inc\Providers\OpenRouterProvider;
...
class OpenRouterTranslationEngine extends TRP_Machine_Translator {
     ...
     */
    protected function get_provider(): ?OpenRouterProvider {
    ...
    public function send_request( $source_language, $language_code, $strings_array ) {
        ...
        if ( ! $provider ) {
            return new WP_Error(
                'provider_error',
                __( 'OpenRouter provider could not be initialized.', 'hollisho-integration-deepseek-for-translatepress' )
            );
        }
    ...
    private function translate_chunk_with_retry( array $chunk, string $source_language, string $target_language ) {
        ...
        if ( ! ( is_array( $response ) && isset( $response['response'] ) &&
            ...
            return new WP_Error(
                ...
                sprintf( 'OpenRouter returned non-200 status for chunk of %d items.', $chunk_size )
            );
        }
        ...
        if ( ! empty( $translation_response->error ) ) {
            return new WP_Error(
                ...
                'OpenRouter API error: ' . ( $translation_response->error->message ?? 'unknown' )
            );
        }
    ...
    /**
     * Extract translated content from OpenRouter response.
     *
    ...
    public function check_api_key_validity() {
        ...
        if ( self::ENGINE_KEY === $translation_engine && ( $this->settings['trp_machine_translation_settings']['machine-translation'] ?? '' ) === 'yes' ) {
            ...
            if ( empty( $api_key ) ) {
                $is_error       = true;
                $return_message = __( 'Please enter your OpenRouter API key.', 'hollisho-integration-deepseek-for-translatepress' );
            }
    ...
    public function get_model(): string {
        ...
        return $settings[ self::FIELD_MODEL ] ?? '';
    }
