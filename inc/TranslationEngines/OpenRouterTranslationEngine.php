...
/**
 * OpenRouter machine translation engine (minimal, Chat Completions API)
...
{
    const ENGINE_KEY    = 'openrouter_translate';
    const FIELD_API_KEY = 'openrouter-api-key';
    const FIELD_MODEL   = 'openrouter-model';

    /**
     * Send translation request to OpenRouter Chat Completions API.
     *
    ...
    /**
     * Parse response from OpenRouter Chat Completions API.
     */
     ...
     * Same contract as DeepSeek/OpenAI: return keyed array, omit failed translations.
     */
    public function translate_array($new_strings, $target_language_code, $source_language_code)
    {
        ...
        // Chunk into batches of 64 (same as DeepSeek/OpenAI)
    ...
    public function check_api_key_validity()
    {
        ...
        if (empty($api_key)) {
            $is_error = true;
            $return_message = 'Please enter your OpenRouter API key.';
        }
